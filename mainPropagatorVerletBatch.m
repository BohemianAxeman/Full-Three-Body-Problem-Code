...
function [tvec, BodyOut] = mainPropagatorVerletBatch(tend,Body,G,varstep,step,tol,filename)
% dbstop if error
BodyOut = Body;
N = length(Body);
...
havg = 0;
maxI = 500;
batchPer = .1; %Percent of simulation finished (used for saving data)
global currTime
...
if varstep == 0 %fixed step size, preallocate exact memory
    ...
    %Variable step size, preallocate dynamic memory
    memChunk = 250000; %create memory in batches of size memChunk
    tvec = zeros(memChunk+1,1);
    for a=1:N
        BodyOut(a).r(2:memChunk+1,1:3)=zeros(memChunk,3);
        BodyOut(a).v(2:memChunk+1,1:3)=zeros(memChunk,3);
        BodyOut(a).psi(2:memChunk+1,1)=zeros(memChunk,1);
        BodyOut(a).w(2:memChunk+1,1:3)=zeros(memChunk,3);
        BodyOut(a).contact(2:memChunk+1,1:N)=zeros(memChunk,3);
    end
end
...
while tvec(i) < tend
    ...
    if varstep == 1 && tvec(end) > 0 %last tvec element will be 0 unless just overwritten
        %We must have maxed out the last element of the body array
        arraySize = length(tvec) + 1;
        newArraySize = arraySize + memChunk;
        tvec(arraySize:newArraySize-1)=zeros(memChunk,1);
        for a=1:N
            BodyOut(a).r(arraySize:newArraySize-1,1:3)=zeros(memChunk,3);
            BodyOut(a).v(arraySize:newArraySize-1,1:3)=zeros(memChunk,3);
            BodyOut(a).psi(arraySize:newArraySize-1,1)=zeros(memChunk,1);
            BodyOut(a).w(arraySize:newArraySize-1,1:3)=zeros(memChunk,3);
            BodyOut(a).contact(arraySize:newArraySize-1,1:N)=zeros(memChunk,3);
        end
    end
    
    % Output data for every 10% of completion 
    if tvec(i)/tend > batchPer
        save(filename, 'BodyOut', 'tvec','-append')
        batchPer = batchPer + .1; 
    end
